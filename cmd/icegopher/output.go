...
type Output interface {
	...
	DescribeProperties(icegopher.Properties)
	Text(string)
	Schema(*icegopher.Schema)
	Spec(icegopher.PartitionSpec)
	Uuid(uuid.UUID)
	Error(error)
}
...
func (t text) DescribeTable(tbl *table.Table) {
	...
	pterm.DefaultTable.
		WithData(pterm.TableData{
			...
			{"Metadata location", tbl.MetadataLoc()},
			{"Table UUID", tbl.Metadata().TableUUID().String()},
			{"Last updated", strconv.Itoa(tbl.Metadata().LastUpdated())},
...
func (text) DescribeProperties(props icegopher.Properties) {
...
func (text) Schema(schema *icegopher.Schema) {
	schemaTree := pterm.LeveledList{}
	var addChildren func(icegopher.NestedField, int)
	addChildren = func(nf icegopher.NestedField, depth int) {
		if nested, ok := nf.Type.(icegopher.NestedType); ok {
			for _, n := range nested.Children() {
...
func (text) Spec(spec icegopher.PartitionSpec) {
	fmt.Println(spec)
}
