...
type commonMetadata struct {
	...
	SchemaList         []*icegopher.Schema       `json:"schemas"`
	CurrentSchemaID    int                       `json:"current-schema-id"`
	Specs              []icegopher.PartitionSpec `json:"partition-specs"`
...
func (c *commonMetadata) Schemas() []*icegopher.Schema { return c.SchemaList }
func (c *commonMetadata) CurrentSchema() *icegopher.Schema {
...
func (c *commonMetadata) PartitionSpecs() []icegopher.PartitionSpec {
	return c.Specs
}
...
func (c *commonMetadata) PartitionSpec() icegopher.PartitionSpec {
	for _, s := range c.Specs {
		if s.ID() == c.DefaultSpecID {
			return s
		}
	}
	return *icegopher.UnpartitionedPartitionSpec
}
...
func (c *commonMetadata) Properties() icegopher.Properties {
	return c.Props
}
...
func (m *MetadataV1) preValidate() {
	if len(m.SchemaList) == 0 {
		m.SchemaList = []*icegopher.Schema{&m.Schema}
	}

	if len(m.Specs) == 0 {
		m.Specs = []icegopher.PartitionSpec{
			icegopher.NewPartitionSpec(m.Partition...)}
