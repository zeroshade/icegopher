...
var (
	TestHeaders = http.Header{
		...
		"User-Agent":       {"github.com/zeroshade/icegopher/" + icegopher.Version},
...
func (r *RestCatalogSuite) TestToken200() {
	...
	cat, err := catalog.NewRestCatalog("rest", r.srv.URL, icegopher.Properties{
		catalog.KeyWarehouseLocation: "s3://some-bucket",
		catalog.KeyCredential:        TestCreds,
	})
	r.Require().NoError(err)

	r.NotNil(cat)
	r.Equal(r.configVals.Get(catalog.KeyWarehouseLocation), "s3://some-bucket")
}
...
func (r *RestCatalogSuite) TestListTables200() {
	...
	cat, err := catalog.NewRestCatalog("rest", r.srv.URL, icegopher.Properties{
		catalog.KeyToken: TestToken,
	})
	r.Require().NoError(err)

	tables, err := cat.ListTables(namespace)
	r.Require().NoError(err)
...
func (r *RestCatalogSuite) TestListTables404() {
	...
	_, err = cat.ListTables(namespace)
...
func (r *RestCatalogSuite) TestListNamespaces200() {
	...
	results, err := cat.ListNamespaces()
	r.Require().NoError(err)
...
func (r *RestCatalogSuite) TestListNamespaceWithParent200() {
	...
	results, err := cat.ListNamespaces("accounting")
	r.Require().NoError(err)
...
func (r *RestCatalogSuite) TestCreateNamespace200() {
	...
	r.Require().NoError(cat.CreateNamespace(nil, "leden"))
}
...
func (r *RestCatalogSuite) TestCreateNamespace409() {
	...
	err = cat.CreateNamespace(nil, "fokko")
...
func (r *RestCatalogSuite) TestDropNamespace404() {
	...
	err = cat.DropNamespace("examples")
	r.ErrorIs(err, catalog.ErrNoSuchNamespace)
}
...
func (r *RestCatalogSuite) TestLoadNamespaceProps200() {
	...
	props, err := cat.LoadNamespaceProperties("leden")
	r.Require().NoError(err)
	r.Equal(icegopher.Properties{"prop": "yes"}, props)
}
...
func (r *RestCatalogSuite) TestLoadNamespaceProps404() {
	...
	_, err = cat.LoadNamespaceProperties("leden")
	r.ErrorIs(err, catalog.ErrNoSuchNamespace)
...
func (r *RestCatalogSuite) TestUpdateNamespaceProps200() {
	...
	summary, err := cat.UpdateNamespaceProperties(table.Identifier([]string{"fokko"}),
		[]string{"abc"}, icegopher.Properties{"prop": "yes"})
	r.Require().NoError(err)

	r.Equal(table.PropertiesUpdateSummary{
		Removed: []string{},
		Updated: []string{"prop"},
		Missing: []string{"abc"},
	}, summary)
...
func (r *RestCatalogSuite) TestUpdateNamespaceProps404() {
	...
	_, err = cat.UpdateNamespaceProperties(table.Identifier{"fokko"}, []string{"abc"}, icegopher.Properties{"prop": "yes"})
	r.ErrorIs(err, catalog.ErrNoSuchNamespace)
...
func (r *RestCatalogSuite) TestLoadTable200() {
	...
	tbl, err := cat.LoadTable("fokko", "table")
	r.Require().NoError(err)

	r.Same(tbl.Catalog(), cat)
	r.Equal(catalog.ToIdentifier("rest", "fokko", "table"), tbl.Identifier())
	r.Equal("s3://warehouse/database/table/metadata/00001-5f2f8166-244c-4eae-ac36-384ecdec81fc.gz.metadata.json", tbl.MetadataLoc())
	r.EqualValues(1, tbl.Metadata().Version())
	r.Equal("b55d9dda-6561-423a-8bfc-787980ce421f", tbl.Metadata().TableUUID().String())
	r.Equal(1646787054459, tbl.Metadata().LastUpdated())
	r.Equal(2, tbl.Metadata().LastColumn())
...
func TestTheCatalog(t *testing.T) {
	r, err := catalog.NewRestCatalog("tabular", "http://0.0.0.0:8181", icegopher.Properties{})
	require.NoError(t, err)

	tbl, err := r.LoadTable("default", "test_all_types")
	require.NoError(t, err)
	assert.NotNil(t, tbl)
}
