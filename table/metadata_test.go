...
func TestMetadataV1Parsing(t *testing.T) {
	...
	expected := icegopher.NewSchema(
		0,
		icegopher.NestedField{ID: 1, Name: "x", Type: icegopher.PrimitiveTypes.Int64, Required: true},
		icegopher.NestedField{ID: 2, Name: "y", Type: icegopher.PrimitiveTypes.Int64, Required: true, Doc: "comment"},
		icegopher.NestedField{ID: 3, Name: "z", Type: icegopher.PrimitiveTypes.Int64, Required: true},
	)

	assert.Equal(t, []*icegopher.Schema{expected}, meta.Schemas())
	assert.Zero(t, data.SchemaList[0].ID)
	assert.True(t, meta.CurrentSchema().Equals(expected))
	assert.Equal(t, []icegopher.PartitionSpec{
		icegopher.NewPartitionSpec(icegopher.PartitionField{
			SourceID: 1, FieldID: 1000, Transform: icegopher.IdentityTransform{}, Name: "x",
		}),
	}, meta.PartitionSpecs())

	assert.Equal(t, icegopher.NewPartitionSpec(icegopher.PartitionField{
		SourceID: 1, FieldID: 1000, Transform: icegopher.IdentityTransform{}, Name: "x",
	}), meta.PartitionSpec())
...
func TestParsingCorrectTypes(t *testing.T) {
	...
	assert.IsType(t, &icegopher.Schema{}, meta.SchemaList[0])
	assert.IsType(t, icegopher.NestedField{}, meta.SchemaList[0].Field(0))
	assert.IsType(t, icegopher.PrimitiveTypes.Int64, meta.SchemaList[0].Field(0).Type)
}
...
func TestSortOrderNotFound(t *testing.T) {
	...
	assert.Error(t, err)
	assert.ErrorIs(t, err, table.ErrInvalidMetadata)
	assert.ErrorContains(t, err, "default-sort-order-id 4 can't be found in [{OrderID:3 Fields:[{SourceID:2 Transform:identity Direction:asc NullOrder:nulls-first} {SourceID:3 Transform:bucket[4] Direction:desc NullOrder:nulls-last}]}]")
}
